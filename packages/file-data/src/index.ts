...
const mySpaceApps = 'shaApps'

let cliArgDataPath = global['data'] || cliArgv.data || 'data'
let appDataPath = path.join(mySpaceApps, cliArgDataPath)
...
export const setDataPath = value => {
    ...
    appDataPath = path.join(mySpaceApps, cliArgDataPath)
}
...
const getAppDataPath = () => path.join(basePath, mySpaceApps, global['data'])
...
export const ensureDataDirectories = directory => {
    ...
    const appParts = path.join(mySpaceApps, global['data']).split(path.sep)
...
export class FileDataRepository<T> {
    ...
    public saveData = (newData: Partial<T>) => {
        console.log('Save data to config file')
        this.inWrite = true
    ...
    private readData = () => {
        try {
            ...
            if (JSON.stringify(newData) !== JSON.stringify(this.data)) {
                ...
                console.log('Dispatch config with value')
                this.signalChange.dispatch(this.data)
            }

        } catch (e) {
            console.log('file is corrupted', this.fullFilePath)
