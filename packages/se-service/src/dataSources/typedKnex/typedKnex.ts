...
export interface ITypedQueryBuilder<ModelType, Row> {
    ...
    whereRaw: IWhereRaw<ModelType, Row>;
    ...
    orderByRaw: IWhereRaw<ModelType, Row>;
    ...
    insert(newObject: Partial<ModelType>, fieldToReturn?: string): Promise<any>;
    countResult(): Promise<number>;
    delById(id: string): Promise<void>;
    update(id: string | number, item: Partial<ModelType>): Promise<void>;
    increment(id: number, columnName: string, amount: number): Promise<void>;
    decrement(id: number, columnName: string, amount: number): Promise<void>;
}
...
export interface IWhereRaw<Model, Row> {
   (clause: string): ITypedQueryBuilder<Model, Row>;
}
...
export class TypedQueryBuilder<ModelType, Row = {}> implements ITypedQueryBuilder<ModelType, Row> {
    ...
    public async insert(newObject: Partial<ModelType>, returningField?: string) {
        return await this.insertItems([newObject], returningField);
    }

    public async insertItems(items: Partial<ModelType>[], returningField?: string) {
        items = [...items];
        ...
        while (items.length > 0) {
            ...
            if (returningField)
                return await this.knex.from(this.tableName).insert(chunk).returning(returningField)
            else
                return await this.knex.from(this.tableName).insert(chunk)
        }
    }
    ...
    public async update(id: string, item: Partial<ModelType>) {
        ...
        await this.queryBuilder.updated(item).where('id', id);
    }

    public async increment(id: number, columnName: string, amount: number) {
        // if (beforeUpdateTransform) {
        //     item = beforeUpdateTransform(item, this);
        // }

        await this.queryBuilder.increment(columnName, amount).where('id', id);
    }

    public async decrement(id: number, columnName: string, amount: number) {
        // if (beforeUpdateTransform) {
        //     item = beforeUpdateTransform(item, this);
        // }

        await this.queryBuilder.decrement(columnName, amount).where('id', id);
    }
    ...
    public orderByRaw(arg: string) {
        this.queryBuilder.orderByRaw(arg)
        return this
    }
    ...
    public whereRaw(clause: string) {
        this.queryBuilder.whereRaw(clause);
        return this;
    }
